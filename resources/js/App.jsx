    ...
    <main className="shell">
      <header className="topbar">
        <div className="brand">
          ...
          <div><h1>بازار طلای ارنوکسین</h1><p>پایش قیمت طلا و سکه با داده‌های {config.sourceName}</p></div>
        </div>
        <div className="actions">
          ...
          <button className="iconButton" onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')} title="تغییر پوسته" aria-label="تغییر پوسته">
      ...
      <section className="hero">
        <div>
          ...
          <p>نمای موبایل‌محور برای بررسی روند، تغییرات تاریخی و مقایسه سریع بازار.</p>
        </div>
        <div className="stats">
      ...
      <section className="layout">
        <aside className="marketPanel">
          ...
          <div className="search"><Search size={18} /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="جستجوی طلا یا سکه" /></div>
          <div className="itemList">
            {filtered.map((item) => <MarketItem key={item.id} item={item} active={selected?.id === item.id} onClick={() => setSelectedId(item.id)} />)}
            {status !== 'loading' && filtered.length === 0 && <div className="empty">داده‌ای برای نمایش وجود ندارد.</div>}
          </div>
        </aside>

        <section className="chartPanel">
          <div className="chartHeader">
            <div><span>{selected ? categoryLabel(selected.category) : 'بازار'}</span><h3>{selected?.name || 'داده‌ای انتخاب نشده است'}</h3></div>
            <div className="range">{ranges.map((d) => <button key={d} className={range === d ? 'active' : ''} onClick={() => setRange(d)}>{formatNumber(d)} روز</button>)}</div>
          </div>
          ...
          <div className="analyticsGrid">
            ...
            <Metric value={analytics?.changePercent} label="بازده بازه ٪" compact tone={analytics?.changePercent < 0 ? 'down' : 'up'} />
          </div>
            ...
            {history.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={history} margin={{ top: 12, right: 10, left: 0, bottom: 0 }}>
                  <defs><linearGradient id="goldGradient" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="var(--gold)" stopOpacity="0.55" /><stop offset="100%" stopColor="var(--gold)" stopOpacity="0.02" /></linearGradient></defs>
                  <CartesianGrid strokeDasharray="3 3" vertical={false} />
                  <XAxis dataKey="time" hide />
                  <YAxis orientation="right" width={82} tickFormatter={(v) => formatNumber(v / 1_000_000)} />
                  <Tooltip content={<ChartTooltip />} />
                  <Area type="monotone" dataKey="current" stroke="var(--gold)" strokeWidth={3} fill="url(#goldGradient)" isAnimationActive />
                </AreaChart>
              </ResponsiveContainer>
            ) : (
              <div className="chartEmpty"><WalletCards size={30} /><span>برای این بازه هنوز تاریخچه‌ای ثبت نشده است.</span></div>
            )}
          </div>
        </section>
      </section>
...
function Metric({ value, label, compact = false, tone = '' }) {
  return <div className={`metric ${compact ? 'compact' : ''} ${tone}`}><strong>{formatNumber(value)}</strong><span>{label}</span></div>;
}
...
function MarketItem({ item, active, onClick }) {
  ...
  return (
    <button className={`marketItem ${active ? 'active' : ''}`} onClick={onClick}>
      ...
      <span className={positive ? 'badge up' : 'badge down'}>{positive ? <TrendingUp size={14} /> : <ArrowDown size={14} />}{formatNumber(item.percent)}٪</span>
    </button>
  );
}
...
function ChartTooltip({ active, payload, label }) {
  ...
  return <div className="tooltip"><span>{formatDate(label)}</span><strong>{formatNumber(payload[0].value)}</strong></div>;
}

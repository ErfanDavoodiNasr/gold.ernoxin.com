...
4. اطلاعات دیتابیس را در `.env.example` وارد کنید (یا بعد از نصب در `.env`).
5. دامنه را باز کنید؛ برنامه `.env.example` را به `.env` کپی می‌کند، `APP_KEY` را مقداردهی می‌کند و جدول‌ها را آماده
   می‌کند.
...
اگر `.env` وجود نداشته باشد، برنامه در اولین اجرا آن را از `.env.example` می‌سازد. اگر `APP_KEY` خالی باشد، در همان اجرا

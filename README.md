...
داشبورد فارسی قیمت طلا و سکه برای بازار ایران. پروژه قیمت‌ها را از منبع مشخص دریافت می‌کند، در MySQL ذخیره می‌کند و در کنار نمودارهای تاریخی، یک بلاگ آموزشی برای توضیح مفاهیم بازار طلا دارد.
...
- PHP `8.2`
- MySQL
- افزونه‌های PHP: `dom`, `pdo`, `pdo_mysql`, `mbstring`, `openssl`, `tokenizer`, `xml`, `ctype`, `json`, `fileinfo`, `curl`

## نصب سریع روی cPanel

1. یک دیتابیس MySQL و user بسازید.
2. اطلاعات دیتابیس و دامنه را در `.env` تنظیم کنید.
3. کل پروژه را با ساختار فعلی داخل مسیر سایت آپلود کنید.
4. دامنه را باز کنید تا برنامه `APP_KEY`، جدول‌ها و مسیرهای لازم را آماده کند.
5. Cron دریافت قیمت را فعال کنید.
...
مسیر `/home/USER/gold/artisan` را با مسیر واقعی پروژه روی هاست جایگزین کنید. اگر مسیر PHP روی هاست متفاوت است، به جای `/usr/local/bin/php` همان مسیر را بگذارید.

اگر هاست اجرای هر دقیقه را محدود کرده، Cron را هر پنج دقیقه اجرا کنید. در این حالت حتی اگر `ESTJT_FETCH_INTERVAL_MINUTES=1` باشد، دریافت واقعی حداکثر هر پنج دقیقه انجام می‌شود.

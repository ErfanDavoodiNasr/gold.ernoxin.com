...
- اگر Cron هر دقیقه باشد و `ESTJT_FETCH_INTERVAL_MINUTES=5` باشد، برنامه هر دقیقه بررسی می‌کند ولی فقط هر پنج دقیقه
  fetch واقعی انجام می‌دهد.
- اگر Cron هر پنج دقیقه باشد و `ESTJT_FETCH_INTERVAL_MINUTES=1` باشد، fetch واقعی حداکثر هر پنج دقیقه انجام می‌شود، چون
  Cron زودتر برنامه را بیدار نمی‌کند.
